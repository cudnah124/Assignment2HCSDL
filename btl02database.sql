DROP DATABASE IF EXISTS TAKEAWAY_CAFE;
CREATE DATABASE IF NOT EXISTS TAKEAWAY_CAFE;
USE TAKEAWAY_CAFE;
UPDATE user SET authentication_string=PASSWORD('1') WHERE User='root';
FLUSH PRIVILEGES;
...
CREATE TABLE GomDH_NuocUong (
    ...
    constraint FK_GomDHNuocUong_MaDonHang FOREIGN KEY (MaDonHang) REFERENCES DonHang(MaDonHang)
    on update cascade
    on delete cascade,
    constraint FK_GomDHNuocUong_KichThuocDoUong FOREIGN KEY (MaMon, KichThuoc) 
    REFERENCES KichThuocDoUong(MaMon, KichThuoc)
    on update cascade
    on delete cascade
);
